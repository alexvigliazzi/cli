...
function upsertHook(
  ...
  for (const matcherValue of matchers) {
    if (!isRecord(matcherValue)) continue;
    const currentMatcher = typeof matcherValue.matcher === 'string' ? matcherValue.matcher : '';
    if (currentMatcher !== matcherName) continue;

    const hooksArray = Array.isArray(matcherValue.hooks) ? matcherValue.hooks : [];
    if (!Array.isArray(matcherValue.hooks)) matcherValue.hooks = hooksArray;

    const alreadyExists = hooksArray.some((h) =>
      isRecord(h) && h.type === 'command' && h.command === command,
    );
...
export async function removeSessionHooks(repoRoot: string): Promise<{ settingsPath: string; removed: boolean }> {
  const settingsPath = path.join(repoRoot, '.claude', 'settings.json');
  ...
  if (!isRecord(hooks)) return { settingsPath, removed: false };
  ...
  for (const eventKey of Object.keys(hooks)) {
    ...
    for (const matcher of matchers) {
      ...
      matcher.hooks = (matcher.hooks as unknown[]).filter((h) => {
        ...
        return typeof h.command !== 'string' || !isSessionsHookCommand(h.command);
      });

      if ((matcher.hooks as unknown[]).length < originalLength) removed = true;
    }
