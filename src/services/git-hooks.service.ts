...
class GitHooksService {
   ...
   */
  async install(gitRoot: string): Promise<{ installed: string[]; alreadyInstalled: string[] }> {
  ...
  async uninstall(gitRoot: string): Promise<{ removed: string[] }> {
    ...
    const prepareResult = await this.removeHook(gitRoot, 'prepare-commit-msg');
    if (prepareResult) removed.push('prepare-commit-msg');

    const postResult = await this.removeHook(gitRoot, 'post-commit');
    if (postResult) removed.push('post-commit');
  ...
  private async removeHook(gitRoot: string, hookName: string): Promise<boolean> {
    const hookPath = path.join(gitRoot, '.git', 'hooks', hookName);
    ...
    if (!content.includes(KODY_CHECKPOINT_MARKER)) return false;

    const lines = content.split('\n');
    const startIdx = lines.findIndex((line) => line.trim() === KODY_CHECKPOINT_MARKER);
    if (startIdx === -1) return false;

    const endIdx = lines.findIndex((line, idx) => idx > startIdx && line.trim() === KODY_CHECKPOINT_END_MARKER);
